...
class Robot(object):
    ...
    def set_robot_direction(self, direction):
        ...
        """
        self.direction = direction % 360 # must be < 360 degrees
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        # random uniform distribution of entire square
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        m = math.floor(pos.get_x()) # get x pos
        n = math.floor(pos.get_y()) # get y pos
        return self.is_tile_furnished(m, n) # classify as furnished
...
class FaultyRobot(Robot):
    ...
    def gets_faulty(self):
        ...
        """
        if random.random() < FaultyRobot.p:
            return True
        return False
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for _ in range(num_trials): # loop num trials times
        room = EmptyRoom(width, height, dirt_amount) # create new room
        robots = [robot_type(room, speed, capacity) for _ in range(num_robots)] # store robots
...
# the graphs show that the faulty robot takes much more time than the standard robot,
# which is expected as the faulty robot is not as reliable.
...
# Both robots have increased time with higher aspect ratios, however the faulty robot consistently does worse than the standard robot,
# which is expected as, again, the faulty robot is unreliable.
