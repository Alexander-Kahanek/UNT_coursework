...
def get_word_score(word, n):
    ...
    returns: int >= 0
    """
    word = list(str.lower(word)) # lowers word
    score = [] # holds score values
    for letter in word:
        score.append(SCRABBLE_LETTER_VALUES.get(letter))
    letter_points = sum(score) # sums score

    word_score = 7*len(word) - 3*(n - len(word)) # math for points
    if word_score < 1:
        word_score = 1 # min of score 1
...
def display_hand(hand):
    ...
    for letter in hand.keys():
        for j in range(hand[letter]):
             print(letter, end=' ') # prints on same line
    print()
...
def deal_hand(n):
    ...
    hand = {}
    nVowels = int(math.ceil(n / 3)) # number of vowels to include

    for i in range(nVowels - 1):
        x = random.choice(VOWELS) # randomly choose vowels
        hand[x] = hand.get(x, 0) + 1
    
    for i in range(nVowels, n):
        x = random.choice(CONSONANTS) # randomly choose constants
        hand[x] = hand.get(x, 0) + 1
...
def update_hand(hand, word):
    ...
    """
    new_hand = hand.copy()
    word = str.lower(word) # lowers word
    ...
    if '*' not in word:
        for i in range(len(word)):
            if word[i] in hand and hand[word[i]] >= word.count(word[i]) and word in word_list:
                test.append(1) # letter and word is valid
            else:
                test.append(0) # letter or word is not valid
        if sum(test) == len(word):
            return True # word is contained in hand and valid
        else:
            return False # word is not the above
    elif '*' in word_copy: # there is a * value
            ...
            if word_copy.replace('*', VOWELS[i]) in word_list:
                ...
                for j in range(len(word_copy)):
                    if word_copy[j] in hand and hand[word_copy[j]] >= word_copy.count(word_copy[j]):
                        ...
                        test.append(0) # letter and word is not valid
                if sum(test) == len(word_copy):
                    return True # word is contained in hand and valid
                else:
                    return False # word is not contained in hand nor valid
            else:
                test_list.append(0)
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    total_score = 0 # counter for total score
    while True:
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    nSub = 1
    nReplay = 1
    nHands = int(input('Enter total number of hands: '))
    TOTAL = 0
    while nHands >= 1:
        new_score = 0
